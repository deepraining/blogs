...
lila 的哲学是一个命令做完所有事情，所以在命令不是很多，但基本上都能物尽其用。
...
查看每个文件模块的大小，确定那些比较大的文件需要单独分割出来加载，以提高加载速度。

### 3.3 配置

项目的配置在项目根目录下的 `lila.config.js` 中，每个模块的自定义配置在模块工作空间的下的 `config.js` 文件中。

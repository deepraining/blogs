...
现在，基本上前端的项目打包都会用上 `webpack`，因为 `webpack` 提供了无与伦比的强大的功能和生态。但在创建一个项目的时候，总是免不了要配置 `webpack`，很是麻烦。
...
const run = (config, cb) => {
  webpack(config, (err, stats) => {
    ...
    // 因为每个页面导出的 html 文件都是 index.html 如果不重新命令，会被覆盖掉

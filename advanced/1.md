...
随着前端组件化的发展，组件化框架的更新，如 [react](https://github.com/facebook/react)、[vue](https://github.com/vuejs/vue)，把整个组件的资源进行封装，并只对外暴露一个对象，而调用者无需关心组件的内部实现和资源，直接调用这个对象就够了。
...
这样做的目的，一是 css 的模块化，二是直接绑定组件。

比如，[material-ui](https://github.com/mui-org/material-ui) 便是使用的这种方式。

这里主要介绍一下 [styled-jsx](https://github.com/zeit/styled-jsx)。
